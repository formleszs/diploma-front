  ...
  if (notFound || !lecture) {
    return (
      <div className="rounded-[28px] bg-surface p-8 text-center">
        <p className="font-body text-violet">Лекция не найдена.</p>
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        <div className="rounded-[28px] border border-white/10 bg-surface p-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-label text-xs uppercase text-violet/80">Summary</span>
          </div>
          <p className="mt-2 font-body text-sm text-violet">Краткое резюме по лекции</p>
          <Button type="button" className="mt-4 w-full rounded-xl bg-lime text-violet hover:bg-lime-dark" onClick={handleSummary} disabled={summaryLoading}>
            {summaryLoading ? 'Загрузка…' : 'Получить саммари'}
          </Button>
          {summaryText !== null && (
            <div className="mt-4 rounded-xl border border-violet/10 bg-violet/5 p-4">
              <pre className="whitespace-pre-wrap break-words font-body text-sm text-violet">{summaryText}</pre>
            </div>
          )}
        </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-surface p-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-label text-xs uppercase text-violet/80">Flashcards</span>
          </div>
          <p className="mt-2 font-body text-sm text-violet">Карточки для запоминания</p>
          <Button type="button" className="mt-4 w-full rounded-xl bg-lime text-violet hover:bg-lime-dark" onClick={handleFlashcards}>
            Сгенерировать карточки
          </Button>
          {flashcardsMessage && <p className="mt-4 font-body text-sm text-violet/70">{flashcardsMessage}</p>}
        </div>

        <div
          className={`rounded-[28px] border border-white/10 p-6 transition-colors ${
            quizLocked ? 'cursor-not-allowed bg-surface/70 opacity-90' : 'bg-surface'
          }`}
        >
          {quizLocked ? (
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="block">
                  <div className="flex items-center gap-2 text-violet/50">
                    <HelpCircle size={20} className="text-violet/40" />
                    <span className="font-label text-xs uppercase">Quiz</span>
                  </div>
                  <p className="mt-2 font-body text-sm text-violet/50">Проверка знаний</p>
                  <div className="mt-4 flex w-full items-center justify-center rounded-xl border border-violet/20 bg-violet/10 py-3 font-body text-sm text-violet/50">
                    Открыть квиз
                  </div>
                </div>
              </TooltipTrigger>
            ...
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="font-label text-xs uppercase text-violet/80">Quiz</span>
              </div>
              <p className="mt-2 font-body text-sm text-violet">Проверка знаний</p>
              <Button type="button" className="mt-4 w-full rounded-xl bg-lime text-violet hover:bg-lime-dark" onClick={handleQuiz}>
                Открыть квиз
              </Button>
            </>
          )}
